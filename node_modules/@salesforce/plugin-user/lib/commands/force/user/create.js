...
const permsetsStringToArray = (fieldsPermsets) => {
    if (!fieldsPermsets)
        return [];
    return ts_types_1.isArray(fieldsPermsets)
        ? fieldsPermsets
        : fieldsPermsets.split(',').map((item) => item.replace("'", '').trim());
};
const standardizePasswordToBoolean = (input) => {
...
class UserCreateCommand extends command_1.SfdxCommand {
    ...
    async run() {
        ...
        return {
            ...
            fields: { ...kit_1.mapKeys(fieldsWithoutPermsets, (value, key) => key.toLowerCase()) },
        };
    }
    async catchCreateUser(respBody, fields) {
        ...
        const errMessage = ts_types_1.getString(respBody, 'message') || 'Unknown Error';
        ...
        if (errMessage.includes('LICENSE_LIMIT_EXCEEDED')) {
            ...
            throw core_1.SfdxError.create('@salesforce/plugin-user', 'create', 'licenseLimitExceeded', [profile.Name]);
        }
        else if (errMessage.includes('DUPLICATE_USERNAME')) {
            throw core_1.SfdxError.create('@salesforce/plugin-user', 'create', 'duplicateUsername', [fields.username]);
        }
        else {
            throw core_1.SfdxError.wrap(errMessage);
        }
    }
    ...
    async aggregateFields(defaultFields) {
        ...
        if (this.flags.definitionfile) {
            const content = (await core_1.fs.readJson(this.flags.definitionfile));
