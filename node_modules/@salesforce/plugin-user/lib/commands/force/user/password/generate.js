    ...
    async run() {
        ...
        for (const aliasOrUsername of this.usernames) {
            try {
                ...
                const username = (await core_1.Aliases.fetch(aliasOrUsername)) || aliasOrUsername;
                ...
                if (err.message.includes('Cannot set password for self') ||
                    ...
                    if (parseInt(await org.retrieveMaxApiVersion(), 10) >= 51) {
                        throw new core_1.SfdxError(messages.getMessage('noSelfSetError'), 'noSelfSetError', messages.getMessage('noSelfSetErrorActions').split(os.EOL));
                    }
                    throw new core_1.SfdxError(messages.getMessage('noSelfSetErrorV50'), 'noSelfSetError');
                }
