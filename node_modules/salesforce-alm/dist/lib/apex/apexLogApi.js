...
const StreamClient = require("../core/status");
...
const TRACE_FLAG_QUERY = 'SELECT Id, DebugLevelId, StartDate, ExpirationDate FROM TraceFlag ' +
...
class ApexLogApi {
    ...
    tail() {
        ...
        return new Promise((resolve, reject) => stream.subscribe(TOPIC, this._streamingCallback.bind(this), true).catch((err) => {
            stream.disconnect();
            reject(err);
        }));
    }
    // setup or update traceflag need to stream logs
    async prepareTraceFlag(debugLevel) {
    ...
    async _getDebugLevelId(debugLevel) {
    ...
    async _handleTailMessage(logId) {
    ...
    _applyColor(colorFn, logLinePart) {
    ...
    async getLogById(logId) {
        const conn = await Config.getActiveConnection({ org: this.org });
        const geturl = util.format('%s/services/data/v%s/tooling/sobjects/ApexLog/%s/Body', conn.instanceUrl, conn.version, logId);
        let log = '';
     ...
     */
    async getRecentLogRecords(numOfRecentLogs) {
        const conn = await Config.getActiveConnection({ org: this.org });
        const geturl = this._createLogListUrl(conn, numOfRecentLogs);
        let logs = [];
        await this._getReq(conn, geturl, function (response) {
            logs = response.records;
        });
        return logs;
    }
    _createLogListUrl(conn, numOfRecentLogs) {
        const limitQuery = numOfRecentLogs && numOfRecentLogs > 0 ? `+DESC+LIMIT+${numOfRecentLogs}` : '';
        return util.format('%s/services/data/v%s/tooling/query/?q=' +
            'Select+Id,+Application,+DurationMilliseconds,+Location,+LogLength,+LogUser.Name,+Operation,+Request,StartTime,+Status+' +
            'From+ApexLog+' +
            'Order+By+StartTime' +
            '%s', conn.instanceUrl, conn.version, limitQuery);
    }
    async _getReq(conn, geturl, callback) {
        const logger = this.logger;
        await conn.request(geturl, undefined, function (err, response) {
            if (err) {
                logger.error(err);
...
exports.ApexLogApi = ApexLogApi;

//# sourceMappingURL=apexLogApi.js.map
