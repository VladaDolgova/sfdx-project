...
const keyChainImpls = require("./keyChainImpl");
...
const KeychainPromises = {
    ...
    migrate(oldKeychain, newKeychain, service, account) {
        return KeychainPromises.get(oldKeychain, service, account).then((passwordResult) => KeychainPromises.set(newKeychain, service, account, passwordResult.password));
    },
};
/**
        ...
        return this.keychainImpls
            ...
            return KeychainPromises.get(keychainImpl, KEY_NAME, ACCOUNT)
                ...
                if (err.name === 'PasswordNotFound') {
                    ...
                    return KeychainPromises.set(keychainImpl, KEY_NAME, ACCOUNT, key).then(() => this.init('KEY_SET', platform));
                }
                else {
                    throw err;
                }
            ...
            if (err.name === keyChainImpls.DEPRECATED_KEYCHAIN) {
                ...
                return KeychainPromises.migrate(err.fromKeychain, err.toKeychain, KEY_NAME, ACCOUNT).then(() => this.init('MIGRATED', platform));
            }
            throw err;
        });
    }
