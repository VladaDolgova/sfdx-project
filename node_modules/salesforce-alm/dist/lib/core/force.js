...
const _jwtAuthorize = function (token, orgInstanceAuthority) {
    ...
    return (oauth2
        ...
        .then((localConnection) => localConnection.query(`SELECT Username FROM User WHERE Id = '${connection.userInfo.id}'`))
        .then((response) => ({
        orgId: connection.userInfo.organizationId,
        username: response.records[0].Username,
        accessToken: connection.accessToken,
...
Force.prototype.jwtAuthorize = function (oauthConfig) {
    return Promise.all([fsReadFile(oauthConfig.privateKeyFile, 'utf8'), core_1.getJwtAudienceUrl(oauthConfig)])
        .then(([privateKey, audienceUrl]) => {
        return jwt.sign({
        ...
        .catch((error) => {
        throw core_2.SfdxError.create('salesforce-alm', 'auth', 'JwtGrantError', [error.message]);
    });
};
...
Force.prototype.saveOrgAuthData = function (thisLogger, authObject, oauthConfig, isJwt, saveAsDefault, orgApi, trialExpirationDate) {
    ...
    return promise.then((authObject) => {
        if (this.logger.getEnvironmentMode().isDemo()) {
            ...
            return this.query(orgApi, query).then((result) => {
                const trialExpirationDate = result && result.records.length === 1 ? result.records[0].TrialExpirationDate : null;
                if (!trialExpirationDate) {
                    ...
                    return handleDemoModePrompt(authObject.username).then((answer) => {
                        if (answer.toUpperCase() === 'YES' || answer.toUpperCase() === 'Y') {
                            return this.saveOrgAuthData(this.logger, authObject, oauthConfig, isJwt, saveAsDefault, orgApi, trialExpirationDate);
                        }
                        return BBPromise.reject(possibleError);
                    });
                }
...
Force.prototype.mdapiRestDeploy = function (orgApi, zipStream, options) {
    ...
    return this._getConnection(orgApi, this.config)
        ...
        return `${connection.instanceUrl.replace(/\/$/, '')}/services/data/v${this.config.getApiVersion()}/metadata/deployRequest`;
    })
