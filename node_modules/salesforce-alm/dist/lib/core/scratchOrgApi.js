...
const OrgInfo = require("../org/scratchOrgInfoApi");
...
const _groupOrgs = async function (configsAcrossDevHubs, orgClass, excludeProperties) {
    ...
    return BBPromise.all([
        orgClass
            .create(undefined, core_1.Config.DEFAULT_DEV_HUB_USERNAME)
            .then((org) => org.name)
            .catch(() => null),
        orgClass
            .create(undefined, core_1.Config.DEFAULT_USERNAME)
            .then((org) => org.name)
            .catch(() => null),
...
const _updateOrgIndicators = function (groupedOrgs, _orgUpdate) {
    return BBPromise.map(groupedOrgs.scratchOrgs, (meta) => _orgUpdate(meta[1], { devHubUsername: meta[1].devHubUsername }), { concurrency: 1 })
        .catch((err) => {
...
class Org {
     ...
     */
    isWorkspaceOrg() {
        return (this.type === core_1.Config.DEFAULT_USERNAME && this.getName() === this.config.getAppConfigIfInWorkspace()[this.type]);
    }
        ...
        const cleanup = (name) => {
            ...
            return Alias.byValue(name)
                ...
                if ((info.value === name || info.value === alias) && (info.isGlobal() || info.isLocal())) {
                    const config = await core_1.Config.create({ isGlobal: info.isGlobal() });
                ...
                .then(() => {
                core_2.AuthInfo.clearCache(name);
    ...
    static readMapOfLocallyValidatedMetaConfig(userFilenames, concurrency = 3) {
        ...
        const fileDoesntExistFilter = BBPromise.all(_.map(userFilenames, (fileName) => {
            ...
            .filter((statsAndContent) => !_.isNil(statsAndContent[0]) &&
        ...
        return fileDoesntExistFilter
            ...
            return BBPromise.map(fileContentsAndMeta, (fileContentAndMeta) => {
                /**
    ...
    static readLocallyValidatedMetaConfigsGroupedByOrgType(userFilenames, concurrency = 3, excludeProperties) {
        return Org.readMapOfLocallyValidatedMetaConfig(userFilenames, concurrency)
            .then((configsAcrossDevHubs) => Alias.list().then((aliases) => {
            _.forEach(configsAcrossDevHubs.orgs, (org) => {
                org.alias = _.findKey(aliases, (alias) => alias === org.username);
            });
            return configsAcrossDevHubs;
        }))
            .then((configsAcrossDevHubsWithAlias) => _groupOrgs.call(new configApi_1.Config(), configsAcrossDevHubsWithAlias, Org, excludeProperties))
            .then((groupedOrgs) => _updateOrgIndicators.call({
            logger: logger.child('readLocallyValidatedMetaConfigGroupdByOrgType'),
        }, groupedOrgs, (meta, attributesToMerge) => {
            const org = new Org();
