...
class StreamClient {
    ...
    set waitInMinutes(value) {
        this.logger.debug(`Setting streaming timeout to: ${value}m or ${new time_1.Time(value).milliseconds}ms`);
    ...
    subscribe(topic, callback, returnHandshakePromise, timeoutInMillis) {
        return this.orgApi
            .refreshAuth()
             ...
             * exchanges.
             */
            const streamUrl = url_1.resolve(orgData.instanceUrl, `cometd/${apiVersion}`);
            this.logger.debug(`streamUrl: ${streamUrl}`);
            let retVal = streamUrl;
            let subscribeAccept;
            let subscribeReject;
            ...
            this.client = new this.streamingImpl.Client(streamUrl, {
                ...
                cookiesAllowAllPaths: x === undefined ? true : this.env.getBoolean(StreamClient.SFDX_ENABLE_FAYE_COOKIES_ALLOW_ALL_PATHS),
            ...
            let _t = new time_1.Time(this.getRuntimeWait()).milliseconds;
            if (timeoutInMillis) {
                _t = timeoutInMillis;
            }
            if (_t > MAX_TIMEOUT) {
...
StreamClient.SFDX_ENABLE_FAYE_COOKIES_ALLOW_ALL_PATHS = 'SFDX_ENABLE_FAYE_REQUEST_RESPONSE_LOGGING';
StreamClient.SFDX_ENABLE_FAYE_REQUEST_RESPONSE_LOGGING = 'SFDX_ENABLE_FAYE_REQUEST_RESPONSE_LOGGING';
