...
const messages = {
    default: {
        en_US: {
            ...
            communityTopicLongDescription: 'Use the community commands to create and publish a community, and view a list of available templates in you org.',
            ...
            mdDescribeMetadataCommandCliLong: 'Displays details about metadata types enabled for your org. Use this information to identify the ' +
            ...
            mdDescribeMetadataCommandCliHelp: 'The default target username is the admin user for the default scratch org. The username must have ' +
            ...
            mdDescribeMetadataCommandCliResultFileLong: 'The path to the file where the results of the command are stored. Directing the output to a file makes it easier to extract relevant information for your package.xml manifest file. The default output destination is the console.',
            mdDescribeMetadataCommandCliFilterKnown: 'filter metadata known by the CLI',
            mdDescribeMetadataCommandCliFilterKnownLong: 'Filters all the known metadata from the result such that all that is left are the types not yet fully supported by the CLI.',
            mdDescribeMetadataCommandCliInvalidApiVersionError: 'To display metadata from an earlier API version, use --apiversion and specify a positive numerical value less than or equal to the current API version (%s).',
            // mdapi:listmetadata
            mdListmetadataCommandCliDescription: 'display properties of metadata components of a specified type',
            mdListmetadataCommandCliLong: 'Displays properties of metadata components of a specified type. This call is useful when you want to identify ' +
            ...
            mdListmetadataCommandCliHelp: 'The default target username is the admin user for the default scratch org.' +
                '\n\nExamples:' +
            ...
            mdListmetadataCommandCliResultFileLong: 'The path to the file where the results of the command are stored. The default output destination is the console.',
            mdListmetadataCommandCliInvalidApiVersionError: 'To display metadata from an earlier API version, use --apiversion and specify a positive numerical value less than or equal to the current API version (%s).',
            mdListmetadataCommandCliMetadatatype: 'metadata type to be retrieved, such as CustomObject; metadata type value is case-sensitive',
            mdListmetadataCommandCliMetadatatypeLong: 'The metadata type to be retrieved, such as CustomObject or Report. The metadata type value is case-sensitive.',
            mdListmetadataCommandCliFolder: 'folder associated with the component; required for components that use folders; folder names are case-sensitive',
            mdListmetadataCommandCliFolderLong: 'The folder associated with the component. This parameter is required for components that use folders, such as Dashboard, Document, EmailTemplate, or Report. The folder name value is case-sensitive.',
