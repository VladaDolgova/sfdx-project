...
const getDomainRetries = () => _.isNil(process.env.SFDX_DOMAIN_RETRY) ? 240 : _.toInteger(process.env.SFDX_DOMAIN_RETRY);
const _checkLightningDomain = (myDomain, logger, retryCount = 0) => dns
...
const _open = function (context, orgApi) {
    return _getUrl(context, orgApi).then((url) => {
        ...
        if (getDomainRetries() === 0 || core_1.sfdc.isInternalUrl(url)) {
            return act();
        }
        try {
...
module.exports = OrgOpenCommand;

//# sourceMappingURL=orgOpenCommand.js.map
