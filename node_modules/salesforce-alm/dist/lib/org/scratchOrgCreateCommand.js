...
const fs_readFile = util_1.promisify(core_1.fs.readFile);
...
class OrgCreateCommand extends varargsCommand_1.default {
    ...
    async validate(context) {
        ...
        return this.org
            ...
            if (_.isNil(fixedContext.env) && !core_3.sfdc.isInternalUrl(config.instanceUrl)) {
    ...
    async signupOrgWithStreaming(scratchOrgInfoApi, cliContext) {
        ...
        const coreOrg = await core_2.Org.create({ aliasOrUsername: this.org.name });
        const options = new core_2.StreamingClient.DefaultOptions(coreOrg, TOPIC, this.streamProcessor.bind(this));
        if (cliContext.wait) {
        ...
        try {
            const asyncStatusClient = await core_2.StreamingClient.create(options);
    ...
    async signupOrgWithPolling(scratchOrgInfoApi, cliContext, useCliWait) {
        ...
        const options = new core_2.PollingClient.DefaultPollingOptions(async () => {
            if (this.scratchOrgInfoId) {
        ...
        const client = await core_2.PollingClient.create(options);
        const scratchOrgInfo = await this._getScratchOrgInfo(this.org, cliContext);
    ...
    async execute(cliContext, stdinValues) {
        ...
        try {
            ...
            if (e.code == '403' && e.message && e.message.includes('Unknown client')) {
                 ...
                 */
                if (e.name === core_2.StreamingClient.TimeoutErrorType.HANDSHAKE) {
                    scratchOrgInfoId = await this.signupOrgWithPolling(scratchOrgInfoApi, cliContext, true);
                }
                else if (e.name === core_2.StreamingClient.TimeoutErrorType.SUBSCRIBE) {
        ...
        try {
            await remoteSourceTrackingService_1.RemoteSourceTrackingService.getInstance({ username: scratchOrgApi.getName() });
        }
        catch (err) {
        ...
        const postOrgCreateHookInfo = [orgData].map((element) => ({
        ...
        await core_2.Lifecycle.getInstance().emit('postorgcreate', postOrgCreateHookInfo);
    ...
    async updateRevisionCounterToZero(scratchOrgApi, orgData) {
      ...
      "batchRequests" : [ ${requestBody} ]}`;
            ...
            try {
                ...
                throw new core_2.SfdxError(message, 'SourceStatusResetFailure');
            }
        }
    }
    /**
