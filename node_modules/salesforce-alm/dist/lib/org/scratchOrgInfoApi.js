            ...
            if (scratchOrgInfoComplete.Snapshot) {
                ...
                return scratchOrg.getConfig().then((decryptedConfig) => {
                    ...
                    return scratchOrg.saveConfig(decryptedConfig).then((orgConfigData) => {
                        logger.debug(`_authorize - updated orgConfig: ${util_1.inspect(orgConfigData)}`);
...
signup.checkOrgDoesntExists = async function (_scratchOrgInfo) {
...
signup.prototype.request = async function (scratchOrgInfo) {
    ...
    return core_1.Logger.child('scratchOrgInfoApi').then((logger) => {
        this.logger = logger;
...
signup.prototype.retrieveScratchOrgInfo = async function (scratchOrgInfoId) {
    ...
    if (!this.logger) {
        this.logger = await core_1.Logger.child('scratchOrgInfoApi');
    }
    ...
    switch (request.Status) {
        ...
        case 'Error':
            ...
            error = new core_1.SfdxError(message, 'RemoteOrgSignupFailed');
            break;
        default:
            message = messages().getMessage('signupUnexpected');
            error = new core_1.SfdxError(message, 'UnexpectedSignupStatus');
    }
    throw error;
};
/**
...
signup.prototype.processScratchOrgInfoResult = function (scratchOrgInfoResult, clientSecret, scratchOrg, setAsDefault) {
    ...
    return _authorize(scratchOrgInfoResult, this.force, this.hubOrg, scratchOrg, clientSecret, setAsDefault, this.logger).then((orgData) => {
        ...
        return BBPromise.all([
            ...
            .then(() => {
            this.logger.debug(`processScratchOrgInfoResult - scratchOrgInfoResult.LoginUrl: ${scratchOrgInfoResult.LoginUrl}`);
            ...
            if (resultData.instanceUrl) {
                this.logger.debug(`processScratchOrgInfoResult - resultData.instanceUrl: ${JSON.stringify(resultData.instanceUrl)}`);
                const options = {
                    timeout: kit_1.Duration.minutes(3),
                    frequency: kit_1.Duration.seconds(10),
                    url: new url_1.URL(resultData.instanceUrl),
                };
                return core_1.MyDomainResolver.create(options)
                    .then((resolver) => resolver.resolve().catch((err) => {
