...
class ShapeRepresentationApi {
    ...
    async create(description = '') {
        return this.force
            ...
            if (err.errorCode && err.errorCode === 'NOT_FOUND' && err['name'] === 'ACCESS_DENIED') {
                return Promise.reject(core_1.SfdxError.wrap(messages.getMessage('create_shape_command_no_crud_access')));
            }
            else {
    ...
    async deleteAll() {
        ...
        try {
            ...
            if (err.errorCode && err.errorCode === 'INVALID_TYPE') {
                ...
                return Promise.reject(core_1.SfdxError.wrap(messages.getMessage('delete_shape_command_no_access', shapeIds)));
            }
            // non-access error
            return Promise.reject(err);
        }
    ...
    async getShapeRepresentation(shapeId) {
        if (this.isShapeId(shapeId)) {
            ...
            return Promise.reject(core_1.SfdxError.wrap(messages.getMessage('shape_get_not_a_shape_id')));
        }
    }
}
