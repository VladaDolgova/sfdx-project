...
class SourceConvertApi {
    ...
    async doConvert(context) {
        ...
        try {
            await core_1.fs.access(rootDir, core_1.fs.constants.R_OK);
        }
        catch (err) {
            // Throw a more helpful error when the rootDir is invalid; otherwise rethrow.
            if (err.code === 'ENOENT') {
                throw new core_2.SfdxError(messages.getMessage('invalidRootDirectory', [rootDir], 'sourceConvertCommand'));
            }
            throw err;
        }
